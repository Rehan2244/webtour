...
export class Tooltip extends EventEmitter {
  ...
  mount() {
    ...
    this.audioContainer.className = 'guided-tour-tooltip-audio';
    this.audioContainer.style.display = 'none';
    content.appendChild(this.audioContainer);
    ...
    if (data.audio) {
      if (!this.audioPlayer) {
        this.audioPlayer = new AudioPlayer({
          autoPlay: data.audioAutoPlay || false,
          showProgress: true,
          showDuration: true,
          showVolumeControl: true,
          showPlaybackSpeed: true,
          theme: data.audioTheme || 'light'
        });
        this.audioPlayer.mount(this.audioContainer);
        
        // Forward audio events
        this.audioPlayer.on('play', () => this.emit('audio-play'));
        this.audioPlayer.on('pause', () => this.emit('audio-pause'));
        this.audioPlayer.on('ended', () => this.emit('audio-ended'));
        this.audioPlayer.on('error', (err) => this.emit('audio-error', err));
      }
      
      this.audioPlayer.setSrc(data.audio);
      this.audioContainer.style.display = 'block';
    } else if (this.audioContainer) {
      this.audioContainer.style.display = 'none';
      if (this.audioPlayer) {
  ...
  async hide() {
    ...
    // Pause audio when hiding tooltip
    if (this.audioPlayer) {
      this.audioPlayer.pause();
    }
